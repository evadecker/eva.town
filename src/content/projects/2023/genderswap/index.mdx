...
<figure>
  ...
  <figcaption>
    **Difficulties organizing in Spotify.** The interface makes it challenging
    to organize songs within large playlists. (Yes, I know sorting by title
    exists, but it's unreliable here—John Denver's "Take Me Home, Country Roads"
    deserves to live beside Judy Collins's "Leaving on a Jet Plane/Take Me Home
    Country Roads"!)
  </figcaption>
</figure>
...
Before I could begin building, I had to get familiar with a new domain—databases! Although I've been building for the web for over a decade now, I've primarily dealt with frontend technology. This personal project was my first time working directly on the backend.
...
For the design, I knew I wanted to allow users to browse by pairs of songs shown together. It wouldn't be too different from browsing "by album" in any other music tool, but rather than displaying a single album cover for each result, I'd display pairs of albums.
...
To communicate the way these covers remix sound, reinterpret lyrics, and reconfigure meaning, I established a visual cue to thread its way through the app: two objects, overlapping and slightly askew. Sharing space, but different. This is represented in the logo and the display of albums on the main grid.
...
Typography is set in [Labil Grotesk](https://www.kometa.xyz/typefaces/labil-grotesk/) by KOMETA Typefaces, a queer font which topples over itself, unable to rest upright, mirroring the off-kilter elements elsewhere in the app.
...
<figure class="no-bleed">
  ...
  <figcaption>
    **Labil Grotesk in use on Genderswap.fm.** I love the way the 'O' falls
    over, ready to roll off the screen.
  </figcaption>
</figure>

While the topsy-turvy contextual alternates were enabled for page-level headings, I disabled them for smaller text to prioritize legibility. Even without leaning, Labil's quirky nature still shines through.

## Little Big Details

As with all my work, I looked for opportunities to introduce small but meaningful details. Here are some of my favorites for Genderswap.fm.
...
<figure class="no-bleed" style="max-width: 502px">
  ![Header typography for a detail page within the app. The text reads: Sonata
  No. 8 in C Minor, Op. 13 “Pathétique”: II. Adagio cantabile. Jun Fukamachi
  covering Ludwig van Beethoven](./readable-urls.png)(quality:100)
  <figcaption>
