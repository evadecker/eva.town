...
datePublished: 2024-05-20 11:53:29-04:00
img:
...
[Astro DB](https://astro.build/db/), recently released, made it a breeze to spin up a database and implement publishing, even for a backend novice like me. But this post isn't a tutorial about how to implement a guestbook. (UX developer [Ryan Trimble](https://ryantrimble.com/) already wrote a [great tutorial](https://ryantrimble.com/blog/creating-a-guestbook-with-astro-db/) about that!)
...
Our interfaces used to be more textured—during the heydey of skeuomorphism in the late 2000s, designers prided themselves on being able to craft realistic-looking wood grain, leather, glass, and other effects.
...
We can also tell the form to preserve implicit newlines on submission by adding `wrap="hard"`, which will guarantee that even if we change our CSS in the future, handcrafted ASCII art will remain intact.
...
There are a few other details to take care of, like showing a warning message when there's no room left for text, but the above code takes care of the basic setup for a fixed-size, fixed-width textarea.

### Get a flatbed scanner

They're cheap nowadays, and produce much better results than photography.

There are two types of scanners: CIS scanners have a very shallow focus, while CCD scanners have a [wider depth of field](http://www.microscopy-uk.org.uk/mag/artjan13/dw-scanner-type.html). (I used a [CIS scanner](https://www.amazon.com/Canon-CanoScan-Lide-Slim-Scanner/dp/B07G5YBS1W?th=1) for this project, but if I needed to buy a new scanner today it would be a CCD!)

I've found that 300 DPI is enough resolution for retina displays. Through trial and error, I also learned that putting a black background behind your light documents will make it much easier to isolate the subject in tools like Photoshop after scanning.

### Be mindful of color contrast

Image backgrounds make it more difficult to measure accessible color contrast ratios for text on top; don't make any backgrounds too high-contrast. If you realize a shade you used was too dark, you can adjust contrast settings after scanning your images in. If there are any busy or loud elements in your object, try to position them in a spot where text is unlikely to overlap.

### Be conscious of file size

The downside to working with images can be increased bandwidth. Use a command line tool like [optipng](https://optipng.sourceforge.net/) to losslessly compress files, or a build tool like Astro's [`<Image />`](https://docs.astro.build/en/guides/images/#image--astroassets) to handle it for you.
...
Embrace experimentation and the freedom of designing things without a screen. There are so many potential areas to bring physical media into digital experiences. It isn't always called for or necessary, but for the right moment, a touch of real, physical texture can go a long way toward making an experience that's a joy to use.
