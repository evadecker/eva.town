...
export default function () {
  // Initialize namespace 'synth' (or use existing one if present)
  ...
  synth.init = function () {
    ...
    function getInstrumentIcon (num) {
      return $instrumentIcon.eq(num);
    }

    function showActiveInstrumentIcon () {
      $instrumentIcon.hide()
      getInstrumentIcon(activeInstrument).show()
    }
    ...
    function hideNoteheads (duration) {
      $('.notehead').hide(duration)
    }
    ...
    function hideKeyboardLetters () {
      $('.key-letter').addClass('fadeout')
      setTimeout(function () {
        $('.key-letter').removeClass('visible')
      }, 2000)
    }

    function pressKey (noteName) {
      var $keyId = $('#' + noteName)
      if ($keyId.hasClass('pressed')) {
    ...
    function pressKeyMatchingCoords (x, y) {
      $key.each(function () {
        var posXStart = $(this).offset().left
        var posYStart = $(this).offset().top - $(window).scrollTop()
        var posXEnd =
          $(this).offset().left + $(this)[0].getBoundingClientRect().width
        var posYEnd =
          $(this).offset().top + $(this)[0].getBoundingClientRect().height
    ...
    function loadAllSounds () {
      ...
      for (var i = 0; i < sounds.instruments.length; i++) {
    ...
    synth.startSynth = function () {
      ...
      $('.instrument-next').click(function (e) {
        e.preventDefault()
        nextInstrument()
      })
      ...
      $instrumentIcon.hover(function () {
        if (!isLoading) {
          this.hide()
          $('.synth-randomize').show()
        }
      })
