...
const SOUNDS_DIRECTORY = 'https://s3-us-west-2.amazonaws.com/s.cdpn.io/532115/'
...
var keys = synth.querySelectorAll('.synth-key')
var instrumentIcons = synth.querySelectorAll('.instrument')
var instrumentIconWrapper = synth.querySelector('.synth-instrument')
var noteheads = synth.querySelectorAll('.notehead')
var keyLetters = synth.querySelectorAll('.key-letter')
var prevArrow = synth.querySelector('.instrument-prev')
var nextArrow = synth.querySelector('.instrument-next')
...
function _bindEvents () {
  ...
  Array.from(keys).forEach(key => {
    ...
    key.addEventListener('touchenter', function (e) {
      e.preventDefault()
      if (isTouching) {
        pressKey(this.id)
      }
  ...
  Array.from(instrumentIcons).forEach(icon => {
    icon.addEventListener('hover', function () {
      if (!isLoading) {
        this.hide()
        synth.querySelector('.synth-randomize').style.display = 'block'
      }
    })
  })

  instrumentIconWrapper.addEventListener('click', function () {
    setRandomInstrument()
  })
