...
export const Synth = () => {
  ...
  const [activeNotehead, setActiveNotehead] = useState<NoteName | null>(null);
  const [pressedKeys, setPressedKeys] = useState<NoteName[]>([]);
  ...
  const loadSounds = () => {
    ...
    // REWRITE:
    // LOAD ALL UI SOUNDS, ALL INSTRUMENT SOUNDS
    // for (const i = 0; i < Object.keys(sounds.ui).length; i++) {
    //   Object.keys(sounds.ui)[i].howl.load();
    // }

    // for (const j = 0; j < sounds.instruments.length; j++) {
  ...
  //   for (let j = 0; j < sounds.instruments.length; j++) {
  ...
  const getPrevInstrument = () => {
    if (activeInstrument === 0) {
      return NUM_INSTRUMENTS;
    } else {
      return activeInstrument - 1;
    }
  };

  const getNextInstrument = () => {
    if (activeInstrument === NUM_INSTRUMENTS) {
      return 0;
    } else {
      return activeInstrument + 1;
    }
  ...
  const nextInstrument = () => {
    setActiveInstrument(getNextInstrument());
    sounds.ui.click.howl.play();
    console.log(activeInstrument);
  };

  const prevInstrument = () => {
    setActiveInstrument(getPrevInstrument());
    sounds.ui.click.howl.play();
  };
  ...
  const pressKey = (note: NoteName) => {
    setPressedKeys([...pressedKeys, note]);
    playNote(note);
    setActiveNotehead(note);
  };
  ...
  const releaseKey = (note: NoteName) => {
  ...
  return (
    <SynthBase onMouseUp={handleMouseUp} onTouchEnd={handleTouchEnd}>
      <SynthKeys
      ...
      />
      <SynthStart isSynthActive={isEnabled} onClick={handleStart} />
    </SynthBase>
  );
};
